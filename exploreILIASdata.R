...
exploration_2 <- function()
{
    source('prepareILIASdata.R')
    assert_that(is.data.frame(data_prep))
    
    data_excl <- data_prep %>%
        filter(.data$value %in% 600,
               .data$date >= as.Date('2020-10-06'),
               .data$date <= Sys.Date()) %>%
        select(.data$date) %>%
        unique
    saveRDS(data_excl, file = file.path('data_ilias', 'ilias_exclusions.rds'))
}
